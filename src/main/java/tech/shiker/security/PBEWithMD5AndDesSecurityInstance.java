...
public class PBEWithMD5AndDesSecurityInstance implements SecurityInstance {
    ...
    public EncryptResult encrypt(String src, String key, String salt, Integer iterations) throws Exception {     // 判断Key是否为16位
        ...
        SecretKey secretKey = SecretKeyFactory.getInstance(SecurityMethod.PBE_WITH_MD5_AND_DES.decryptType()).generateSecret(keySpec);
        AlgorithmParameterSpec paramSpec = new PBEParameterSpec(saltBytes, iterations);
        Cipher cipher = Cipher.getInstance(SecurityMethod.PBE_WITH_MD5_AND_DES.decryptInformation());
